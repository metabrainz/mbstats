...
class TestParser(unittest.TestCase):
    ...
    def test_A(self):
        output = self.call_main(
            ['testing', '--dump-config', '--dry-run', '--log-handler=stdout']
        )
    ...
    def test_parseline(self):
        ...
        row, last_msec, bucket = parseline(
            line, ignore_before=0, bucket_duration=1, last_msec=0
        )
        ...
        expected = {
            ...
            'upstreams': {
                ...
                'status': {'10.2.2.31:65412': {'200': 1}},
                'response_time': {'10.2.2.31:65412': 0.024},
                'response_time_count': {'10.2.2.31:65412': 1},
                'connect_time': {'10.2.2.31:65412': 0.0},
                'connect_time_count': {'10.2.2.31:65412': 1},
                'header_time': {'10.2.2.31:65412': 0.024},
                'header_time_count': {'10.2.2.31:65412': 1},
    ...
    def test_parseline_2(self):
        line = (
            "1|1612013386.275|coverartarchive.org|s|-|502|664|-|49|0.019|10.2.2.23:62080, 10.2.2.37:62080, 10.2.2.40:62080,"
    ...
    def test_parseline_msec_valid(self):
        row, last_msec, bucket = parseline(
            self.sample_line, ignore_before=0, bucket_duration=1, last_msec=0
        )
        self.assertEqual(last_msec, 1568962563.374)
    ...
    def test_parseline_msec_invalid(self):
        ...
        with self.assertRaisesRegex(
            ParseSkip, "^could not convert string to float: 'xxx'"
        ):
            row, last_msec, bucket = parseline(
                line, ignore_before=0, bucket_duration=1, last_msec=0
            )
    ...
    def test_parseline_status_invalid(self):
        ...
        with self.assertRaisesRegex(
            ParseSkip, r"^invalid literal for int\(\) with base 10: 'xxx'$"
        ):
            row, last_msec, bucket = parseline(
                line, ignore_before=0, bucket_duration=1, last_msec=0
            )
    ...
    def test_parseline_upstream_addr_valid_one_redirect(self):
        line = self.get_sample_line(
            PosField.upstream_addr, replace_with='6.6.6.6 : 7.7.7.7'
        )
        row, last_msec, bucket = parseline(
            line, ignore_before=0, bucket_duration=1, last_msec=0
        )
    ...
    def test_parseline_upstream_response_time_valid(self):
        line = self.get_sample_line(
            PosField.upstream_response_time, replace_with='12.34'
        )
        row, last_msec, bucket = parseline(
            line, ignore_before=0, bucket_duration=1, last_msec=0
        )
    ...
    def test_parseline_upstream_connect_time_valid(self):
        line = self.get_sample_line(
            PosField.upstream_connect_time, replace_with='12.34'
        )
        row, last_msec, bucket = parseline(
            line, ignore_before=0, bucket_duration=1, last_msec=0
        )
    ...
    def test_parser_all(self):
        ...
        for i in range(20):
            line = self.get_sample_line(
                PosField.msec, replace_with=str(start), line=sample_line
            )
            if i == 3:
                # just to ensure upstream header time absence gives correct result
                line = self.get_sample_line(
                    PosField.upstream_header_time, replace_with='-', line=line
                )
            elif i == 6:
                line = self.get_sample_line(
                    PosField.upstream_header_time, replace_with='x', line=line
                )
            elif i >= 11 and i <= 13:
                # empty upstream header time, that could lead to division by zero
                line = self.get_sample_line(
                    PosField.upstream_addr,
                    replace_with='10.2.2.31:65412, 10.2.2.32:65412',
                    line=line,
                )
                line = self.get_sample_line(
                    PosField.upstream_status, replace_with='500, 500', line=line
                )
                line = self.get_sample_line(
                    PosField.upstream_response_time,
                    replace_with='0.000, 0.000',
                    line=line,
                )
                line = self.get_sample_line(
                    PosField.upstream_connect_time,
                    replace_with='0.000, 0.000',
                    line=line,
                )
                line = self.get_sample_line(
            ...
            if i % 2:
                line = self.get_sample_line(
                    PosField.status, replace_with='302', line=line
                )
            try:
                row, last_msec, bucket = parseline(
                    line, ignore_before=0, bucket_duration=bucket_duration, last_msec=0
                )
        ...
        self.assertEqual(
            list(map(lambda v: bucket2time(v, bucket_duration), storage)),
            [
                '2019-09-20T06:56:05+00:00',
                '2019-09-20T06:56:10+00:00',
                '2019-09-20T06:56:15+00:00',
                '2019-09-20T06:56:20+00:00',
                '2019-09-20T06:56:25+00:00',
                '2019-09-20T06:56:30+00:00',
                '2019-09-20T06:56:35+00:00',
                '2019-09-20T06:56:40+00:00',
            ],
        )
        ...
        self.assertAlmostEqual(
            mbs['_upstreams_header_time_premean'][upstream_key], 0.0480, places=4
        )
        ...
        # NOTE: float imprecision can lead to a result not exactly equal to expected value
        self.assertAlmostEqual(
            mbs['_upstreams_header_time_premean'][upstream_key], 0.0720, places=4
        )
